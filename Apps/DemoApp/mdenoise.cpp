...
void ReadESRI(FILE* fp, struct ESRIHeader* header)
{
	...
	if(header->isnodata)
	{
		...
		for(i=0; i<header->nrows; i++)
		{
			...
			{
				k = j+i*header->ncols;
				if(fabs(value[k]-header->nodata_value)<FLT_EPSILON)
				{
		...
		for(i=0; i<header->nrows-1; i++)
		{
			for(j=0;j<header->ncols-1;j++)
			{
				...
				if(k==0){
					...
					if((fabs(value[header->index[kk[2]]]-value[header->index[kk[0]]])> 
						fabs(value[header->index[kk[3]]]-value[header->index[kk[1]]]))&& 
						(fabs(value[header->index[kk[1]]]-value[header->index[kk[0]]])> 
						fabs(value[header->index[kk[3]]]-value[header->index[kk[2]]])))
					{
						m_pn3Face[m_nNumFace][0] = header->index[kk[0]];
						m_pn3Face[m_nNumFace][1] = header->index[kk[1]];
						m_pn3Face[m_nNumFace][2] = header->index[kk[2]];
